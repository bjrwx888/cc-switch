...
/// Simplified for Windows to avoid hanging on version checks
pub fn find_claude_binary(app_handle: &tauri::AppHandle) -> Result<String, String> {
    ...
    // On Windows, try to find claude in common locations
    #[cfg(windows)]
    {
        // First try using 'where' to find claude in PATH
            ...
            .output()
        {
            if output.status.success() {
                ...
                if let Some(first_path) = paths.lines().next() {
                    ...
                    if !path.is_empty() {
                        // If the path doesn't end with .cmd or .exe, check for .cmd version
                        let path_lower = path.to_lowercase();
                        if !path_lower.ends_with(".cmd") && !path_lower.ends_with(".exe") {
                            // Try to find the .cmd version
                            let cmd_path = format!("{}.cmd", path);
                            if PathBuf::from(&cmd_path).exists() {
                                info!("Found claude.cmd in PATH: {}", cmd_path);
                                return Ok(cmd_path);
                            }
                        }
        ...
        // Scan common npm global installation paths on Windows
        let search_paths = get_windows_search_paths();
        for search_path in search_paths {
            let claude_cmd = search_path.join("claude.cmd");
            let claude_exe = search_path.join("claude.exe");

            if claude_cmd.exists() {
                info!("Found claude.cmd at: {:?}", claude_cmd);
                return Ok(claude_cmd.to_string_lossy().to_string());
            }
            if claude_exe.exists() {
                info!("Found claude.exe at: {:?}", claude_exe);
                return Ok(claude_exe.to_string_lossy().to_string());
            }
        }

        // Fallback to just "claude" and hope it's in PATH
        warn!("Claude not found in common locations, falling back to PATH lookup");
        return Ok("claude".to_string());
    }

    // On Unix, discover installations
    #[cfg(not(windows))]
    {
        let installations = discover_system_installations();
        if let Some(best) = select_best_installation(installations) {
            info!("Selected Claude: {}", best.path);
            return Ok(best.path);
        }
        Err("Claude Code not found. Please install it with: npm install -g @anthropic-ai/claude-code".to_string())
    }
}

/// Get common search paths for Claude on Windows
#[cfg(windows)]
fn get_windows_search_paths() -> Vec<PathBuf> {
    let mut paths = Vec::new();

    // User's npm global directory
