...
fn create_system_command(claude_path: &str, args: Vec<String>, project_path: &str) -> Command {
    ...
    #[cfg(windows)]
    {
        // Check if this is a .cmd file - if so, we need to run via cmd.exe
        let is_cmd_file = claude_path.to_lowercase().ends_with(".cmd");

        if is_cmd_file {
            // For .cmd files, run through cmd.exe
            ...
            log::info!("Windows command (cmd): cmd /C {} {}", claude_path, args.join(" "));
            ...
            // For .exe files or bare commands, run directly
            ...
            log::info!("Windows command (exe): {} {}", claude_path, args.join(" "));
